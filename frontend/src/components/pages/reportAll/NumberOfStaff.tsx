...
export default function Income({ }: Props) {
  ...
  const [numStaff, setNumStaff] = useState<Staff[]>([]);

  const getNumber = async (month: number | null, year: number | null) => {
    try {
      const response = await fetch(
        `http://localhost:3000/api/getNumberOfStaff/${month}/${year}`
      );
      const data = await response.json();
      console.log(data);
      setNumStaff(data);
      ...
      <div className="hero min-h-screen bg-base-200">
        <div className="hero-content text-center">
          <div className="">
            ...
            <h1 className="text-5xl font-bold">Staff Summary</h1>
            ...
            <div className="stats text-primary-content border-4 border-sky-400 shadow-lg my-2">
              ...
              {numStaff.map((item) => (
                <div className="stat">
                  <div className="stat-title">{item.role_name}</div>
                  <div className="stat-value">{item.number_of_staff}</div>
