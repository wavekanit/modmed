...
export default function ManageStaffPage() {
  ...
  function detailClick(val : any) {
    navigate("/manage_staff/details", { replace: true, state: { val } });
  }
  ...
  async function SearchClick(searching: string) {
    ...
    if (searching === "") {} else {
            try {
              const response = await axios.get(`http://localhost:3000/api/getStaff/${searching}`);
  ...
  function addDoctor() {
    navigate("/manage_staff/staff_insert", { replace: true });
  }
