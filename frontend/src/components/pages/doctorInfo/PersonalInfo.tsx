...
export default function PersonalInfo(props: { id: number }) {
    ...
    function editEducation(val: any) {
        navigate("/manage_doctor/details/education_update", { replace: true, state: { val: val, id : id } });
    }

    function addEducation() {
        navigate("/manage_doctor/details/education_insert", { replace: true, state: { id : id } });
    }

    async function delEducation(val: any) {
        try {
            const response = await axios.post("http://localhost:3000/api/delDoctorEducation", {
                id: id,
                level_edu: val.level_edu,
                diploma: val.diploma,
            });
            alert("Deletion Successfully !");
            navigate("/manage_doctor/details", { replace: true, state: { val: id } });
